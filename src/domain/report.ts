...
export function createReviewReport(analysis: AnalysisResult, reviews: ReviewRecordMap, today: Date): string {
  ...
  const overviewLine = isCrmHistory
    ? `本期共导入 ${overview.projectCount} 个项目，储备金额合计 ${overview.totalAmountWan.toLocaleString()} 万元。跟进中 ${overview.inProgressCount} 个，手动标记呆滞 ${analysis.manualStagnationProjects.length} 个；跟进停滞 ${followUpDelayedCount} 个，签约预期失效 ${expectedSignOverdueCount} 个。`
